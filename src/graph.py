...
def get_fingers_distance(note):
    ...
    new_finger_positions = [
        elem for elem in finger_positions if elem != 0 or elem != 21
    ]
        ...
        for string in range(num_strings):
            for fret in range(num_frets):
                if note[string, fret] == 1:
                    current_chord.append(string, fret)  # type: ignore

        # 新しいノードを追加する
        depressing_fingers_dist = get_fingers_distance(tmp_note)

        distance = abs(prev_fret - (depressing_fingers_dist / 2)) + (
            depressing_fingers_dist
        )
