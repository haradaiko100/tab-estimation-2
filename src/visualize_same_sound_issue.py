...
def plot_tab(tab, note_resolution):
    string_style_dict = {0: "r", 1: "y", 2: "b", 3: "#FF7F50", 4: "g", 5: "#800080"}
    fret_style_dict = {
        0: "#CC0033",
        1: "#3300CC",
        2: "#9900CC",
    ...
    for time in range(len(tab)):
        if time % note_resolution == 0:
            ...
            plt.vlines(
                x=time, ymin=0, ymax=5, colors="k", lw=0.1, ls="dotted", zorder=0
            )
        for string in range(6):
            ...
            if fret != 20:
                plt.scatter(
                    ...
                    s=50,
                    marker="${}$".format(fret),
                    color=pitch_style_list[string, fret],
                    linewidths=1,
                    zorder=5,
                )
...
def visualize(npz_filename_list, kwargs):
    ...
    for npz_filename in npz_filename_list:
        ...
        if mode == "F0":
            ...
            # 異弦同音の出力
            same_sound_issue_tab = npz_file["same_sound_issue_tab"]
            same_sound_issue_pred_tab = npz_file["same_sound_issue_pred_tab"]
            same_sound_issue_graph_tab = npz_file["same_sound_issue_graph_tab"]
        ...
        if input_feature_type == "cqt":
            ...
            librosa.display.specshow(
                librosa.amplitude_to_db(np.abs(cqt)),
                x_axis="time",
                y_axis="cqt_hz",
            ...
            librosa.display.specshow(
                librosa.amplitude_to_db(np.abs(melspec)),
                x_axis="time",
                y_axis="hz",
                sr=down_sampling_rate,
                hop_length=hop_length,
        ...
        if mode == "F0":
            ...
            sns.heatmap(
                (note_gt * 0.4 + note_pred).T, cmap="hot", cbar=False, rasterized=False
            ).invert_yaxis()
            plt.legend(handles=[TP_patch, FP_patch, FN_patch])
            ...
            librosa.display.specshow(
                (frame_F0_gt * 0.4 + frame_F0_from_tab_pred).T,
                x_axis="time",
                y_axis=None,
                sr=down_sampling_rate,
                hop_length=hop_length,
        ...
        for n_layer in range(encoder_layers):
            for n_head in range(encoder_heads):
                ...
                sns.heatmap(
                    attn_map[n_layer, n_head], cmap=cmap, norm=LogNorm(vmin=1e-3)
                )
